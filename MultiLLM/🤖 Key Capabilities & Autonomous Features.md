...
We now use the `send-claude-message.sh` script, which internally leverages `llm_handler.py` for all agent communication with various LLMs:

```bash
# Send message to any LLM agent
...
To configure the LLM used by the orchestrator, modify the `config.yaml` file in the project root. This file allows you to specify the active LLM, model name, and other parameters.
...
**API Key Security**: For production environments, it is highly recommended to set your API key as an environment variable (e.g., `export LLM_API_KEY="your_key_here"`) rather than directly in `config.yaml`.

Supported `active_model` values:
- `litellm`: Recommended for broad LLM support via the LiteLLM library.
- `gemini`: For Google Gemini models.
- `qwen`: For Alibaba Cloud Qwen models.
- `claude`: For Anthropic Claude models.

Make sure to install the necessary Python packages for your chosen LLM (e.g., `pip install litellm google-generativeai anthropic requests PyYAML`).
...
- `llm_handler.py` - Unified LLM communication module
